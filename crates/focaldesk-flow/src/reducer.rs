...
use super::state::{ActiveContext, FocalDesk, OutputId, OutputState, TaskId};
...
pub fn reduce(state: &mut FocalDesk, ev: FlowEvent) -> ReduceResult {
...
fn touch_meta(state: &mut FocalDesk, task: TaskId, now: u64) {
    state
        .task_meta
        .entry(task)
...
fn choose_focus_after_close(state: &mut FocalDesk, rr: &mut ReduceResult, output: OutputId) {
    let out = state.output_mut(output);
