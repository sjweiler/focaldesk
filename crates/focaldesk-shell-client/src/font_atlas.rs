    ...
    pub fn render(
        ...
        for ch in text.chars() {
            ...
            if glyph.w == 0 || glyph.h == 0 {
                cursor_x += (glyph.advance * scale as f32).round() as i32;
                continue;
            }
            ...
            let dst = Rectangle::<i32, Physical>::from_loc_and_size(
                (
                    cursor_x + (glyph.xmin as f64 * scale).round() as i32,
                    baseline_y
                        - (glyph.ymin as f64 * scale).round() as i32
                        - (glyph.h as f64 * scale).round() as i32,
                ),
                (
                    (glyph.w as f64 * scale).round().max(1.0) as i32,
                    (glyph.h as f64 * scale).round().max(1.0) as i32,
                ),
            );
