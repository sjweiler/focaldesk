...
FocalDesk is a retro-futuristic, aerospace-inspired desktop environment built
around a custom Wayland compositor and a cohesive system-experience layer. The
goal is a responsive, keyboard-first UI that remains visually distinctive while
staying technically grounded in measurable latency, stability, and security.
...
The vision describes the intended experience, not the current feature set.
Implemented, experimental, and planned work is tracked in the
[project roadmap](../ROADMAP.md) and the status table in the
